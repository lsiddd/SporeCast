...
# Fortigate Forwarder Deployment Script
# This script deploys the Fortigate log forwarder with configurable settings

set -e

# --- Configuration ---
APP_NAME="fortigate_forwarder"
SERVICE_NAME="fortigate-forwarder"
SOURCE_SERVICE_FILE_NAME="fortigate-forwarder.service"
...
DEFAULT_LOG_FILE="/var/log/fortigate_forwarder.log"
DEFAULT_STATE_DIR="/var/lib/fortigate-forwarder"
DEFAULT_CACHE_DIR="${DEFAULT_STATE_DIR}/threat_intel_cache"

# --- Pre-flight Checks ---
echo "--- Running Pre-flight Checks for Fortigate Forwarder ---"
...
# Validate that the config file specifies Fortigate forwarder
if ! grep -q 'type = "fortigate"' "${PROJECT_ROOT}/${CONFIG_FILE}"; then
    echo "Warning: Configuration file doesn't specify 'fortigate' forwarder type."
    echo "Make sure forwarder.type = \"fortigate\" in your configuration file."
fi
...
echo "--- Creating Fortigate Forwarder Service File ---"

# Create the service file content
cat > "${PROJECT_ROOT}/${SOURCE_SERVICE_FILE_NAME}" << EOF
[Unit]
Description=Fortigate Log Forwarder
Documentation=https://github.com/your-org/fortigate-forwarder
...
SyslogIdentifier=fortigate-forwarder
...
ProtectHome=yes
ReadWritePaths=/var/log /var/lib/fortigate-forwarder /tmp
...
# --- Compilation ---
echo "--- Compiling the Fortigate Forwarder ---"
