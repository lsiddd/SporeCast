...
async fn main() -> Result<()> {
    ...
    // --- Spawn Palo Alto Enrichment Worker Threads ---
    ...
    for i in 0..ENRICHMENT_WORKER_COUNT {
        ...
        let handle = thread::Builder::new()
            .name(format!("palo_enrich_worker_{}", i))
            .spawn(move || {
    ...
    info!("Main task waiting for all Palo Alto enrichment worker threads to complete.");
    drop(raw_log_tx);
    drop(wazuh_raw_tx);
